...
/**
 *
 * @author miniclip22 & fmnabreu & noobshark & PIPOKID & Gil Castro 1110484 &
 * Rita Nogueira 1110494
 */
public class ExpenseRecord {
    ...
    public ArrayList<Expense> getCurrentMonthExpenses() {
        ...
        if (current_month == 1 || current_month == 3 || current_month == 5 || current_month == 7 || current_month == 8 || current_month == 10 || current_month == 12) {
            ...
            if (current_year % 400 == 0 || current_year % 100 != 0 && current_year % 4 == 0) {
                ...
                endDate.setDayOfMonth(28);
            }
        } else {
            endDate.setDayOfMonth(30);
        }
    ...
    /**
     * Returns a list of objects with the type and the amount of expenses of a
     * month.
     *
     * @param month
     * @param year
    ...
    public ArrayList<TypeTotalExpense> getMonthExpensesByType(int month, int year) {
        ...
        for (ExpenseType type : repo.getListOfTypes()) {
            sum = new BigDecimal(0);
            for (Expense exp : new ExpenseTypeWithExpenseIterator(type, expenseRepo.iterator(type, year, month))) {
                sum.add(exp.getAmount());
            }
            result.add(new TypeTotalExpense(type, sum));
        }
